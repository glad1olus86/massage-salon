                ...
                <div class="duty-row__days">
                    @foreach($duty->week_days as $day)
                        <span class="duty-day {{ $day->has_duty ? ($day->is_completed ? 'duty-day--done' : 'duty-day--pending') : 'duty-day--empty' }}" 
                              ...
                              onclick="openDayModal('{{ $day->date->format('Y-m-d') }}', '{{ $duty->branch_id ?? '' }}')">
                        </span>
                    @endforeach
                </div>
...
function openDayModal(date, branchId) {
    currentDate = date;
    ...
    // Используем endpoint календаря админа
    fetch('{{ url("infinitycrm/calendar/day") }}/' + date + '?branch_id=' + branchId, {
