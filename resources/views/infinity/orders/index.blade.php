            ...
            <div class="header-actions">
                <div class="dropdown" data-dropdown>
                    ...
                    <div class="dropdown__menu" role="listbox" aria-label="{{ __('Период') }}">
                        <a href="{{ route('infinity.orders.index', ['period' => 'day']) }}" class="dropdown__option" role="option" {{ ($period ?? '') == 'day' ? 'aria-selected=true' : '' }}>{{ __('день') }}</a>
                        <a href="{{ route('infinity.orders.index', ['period' => 'week']) }}" class="dropdown__option" role="option" {{ ($period ?? 'week') == 'week' ? 'aria-selected=true' : '' }}>{{ __('неделю') }}</a>
                        <a href="{{ route('infinity.orders.index', ['period' => 'month']) }}" class="dropdown__option" role="option" {{ ($period ?? '') == 'month' ? 'aria-selected=true' : '' }}>{{ __('месяц') }}</a>
                        <a href="{{ route('infinity.orders.index', ['period' => 'all']) }}" class="dropdown__option" role="option" {{ ($period ?? '') == 'all' ? 'aria-selected=true' : '' }}>{{ __('все') }}</a>
                    </div>
                </div>
                <a href="{{ route('infinity.orders.create') }}" class="btn btn--dark sm-button sm-button--dark">
                    + {{ __('Новый заказ') }}
                </a>
            </div>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('infinity.orders.edit', $order) }}" class="action-btn action-btn--edit" title="{{ __('Редактировать') }}">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                    </svg>
                                </a>
                                <form action="{{ route('infinity.orders.destroy', $order) }}" method="POST" style="display: inline;" onsubmit="return confirm('{{ __('Удалить заказ?') }}')">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="orders-cards orders-cards--mobile">
                ...
                <div class="order-card">
                    ...
                    <div class="order-card__actions">
                        <a href="{{ route('infinity.orders.edit', $order) }}" class="order-card__btn order-card__btn--edit">
                        ...
                        <form action="{{ route('infinity.orders.destroy', $order) }}" method="POST" class="order-card__form" onsubmit="return confirm('{{ __('Удалить заказ?') }}')">
                            @csrf
                            @method('DELETE')
...
.orders-table tbody td {
    ...
    border-bottom: 1px solid rgba(22, 11, 14, 0.05);
}
.orders-table tbody tr:last-child td {
    border-bottom: none;
}
.orders-table tbody tr:hover {
    background-color: rgba(177, 32, 84, 0.02);
}
.order-amount {
    font-weight: 600;
...
.action-btn--edit {
    background-color: rgba(177, 32, 84, 0.1);
    color: var(--brand-color);
}
.action-btn--edit:hover {
    background-color: rgba(177, 32, 84, 0.2);
}
.action-btn--delete {
...
@push('scripts')
{{-- Dropdown обрабатывается глобальным interactions.js --}}
@endpush
