...
@section('content')
    ...
    <div class="mobile-content">
        ...
        <div class="mobile-section-title">
            <div class="mobile-section-title-left">
                <img src="{{ asset('fromfigma/workplaces.svg') }}" alt="" width="22" height="22"
                     onerror="this.outerHTML='<svg width=22 height=22 viewBox=\'0 0 24 24\' fill=none stroke=#FF0049 stroke-width=2><rect x=2 y=7 width=20 height=14 rx=2 ry=2></rect><path d=\'M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16\'></path></svg>'">
                <span>{{ __('Work Places') }}</span>
            </div>
            @can('create work place')
                <a href="#" data-url="{{ route('work-place.create', ['redirect_to' => 'mobile']) }}" data-ajax-popup="true" 
                   data-title="{{ __('Create Work Place') }}" data-size="md" class="mobile-add-btn">
        ...
        <div class="mobile-search-wrapper mb-3">
            <div class="mobile-search-box">
                ...
                <input type="text" id="liveSearchInput" class="mobile-search-input" 
                       placeholder="{{ __('Search workplaces') }}..." 
                       autocomplete="off">
        ...
        <div id="workplacesList">
            @forelse($workplaces as $workplace)
                <div class="mobile-card workplace-card mb-3" 
                     data-name="{{ strtolower($workplace->name) }}"
                     data-address="{{ strtolower($workplace->address ?? '') }}"
                     onclick="window.location='{{ route('mobile.workplaces.show', $workplace->id) }}'">
                    <div class="d-flex align-items-center">
                        <div class="mobile-workplace-icon me-3">
                            <img src="{{ asset('fromfigma/workplaces.svg') }}" alt="" width="24" height="24"
                                 onerror="this.outerHTML='<svg width=24 height=24 viewBox=\'0 0 24 24\' fill=none stroke=#FF0049 stroke-width=2><rect x=2 y=7 width=20 height=14 rx=2 ry=2></rect><path d=\'M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16\'></path></svg>'">
                        </div>
                        <div class="flex-grow-1">
                            <h6 class="mb-1 workplace-name">{{ $workplace->name }}</h6>
                            <div class="mobile-card-meta">
                                <span class="mobile-badge mobile-badge-employees me-1">
                                    {{ $workplace->current_assignments_count }} {{ __('employees') }}
                                </span>
                                <span class="mobile-badge mobile-badge-address me-1">
                                    <i class="ti ti-map-pin"></i> {{ Str::limit($workplace->address, 20) }}
                                </span>
                                <span class="mobile-badge mobile-badge-positions">
                                    {{ $workplace->positions_count }} {{ __('positions') }}
                                </span>
                            </div>
                        </div>
            ...
            @empty
                <div class="mobile-empty-state">
                    <img src="{{ asset('fromfigma/workplaces.svg') }}" alt="" width="48" height="48" style="opacity: 0.3;"
                         onerror="this.outerHTML='<svg width=48 height=48 viewBox=\'0 0 24 24\' fill=none stroke=#ccc stroke-width=1.5><rect x=2 y=7 width=20 height=14 rx=2 ry=2></rect><path d=\'M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16\'></path></svg>'">
                    <p class="mt-2 text-muted">{{ __('No work places found') }}</p>
                    @can('create work place')
                        <a href="#" data-url="{{ route('work-place.create', ['redirect_to' => 'mobile']) }}" data-ajax-popup="true" 
                           data-title="{{ __('Create Work Place') }}" class="btn btn-sm mobile-btn-primary">
                            {{ __('Create Work Place') }}
                        </a>
                    @endcan
                </div>
            @endforelse
        ...
        <div id="noSearchResults" class="text-center py-5" style="display: none;">
            ...
            <p class="mt-3 text-muted">{{ __('No workplaces match your search') }}</p>
        </div>
    </div>
    ...
    <style>
        ...
        .mobile-badge-employees {
            background: #22B404;
            color: #fff;
        }
        .mobile-badge-address {
            background: #3B82F6;
            color: #fff;
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var workplaceCards = document.querySelectorAll('.workplace-card');
            var noResults = document.getElementById('noSearchResults');
            var originalNames = {};
            
            // Store original names
            workplaceCards.forEach(function(card, index) {
                originalNames[index] = card.querySelector('.workplace-name').innerHTML;
            });
            ...
            searchInput.addEventListener('input', function() {
                ...
                if (query.length < 2) {
                    workplaceCards.forEach(function(card, index) {
                        ...
                        card.querySelector('.workplace-name').innerHTML = originalNames[index];
                    });
                ...
                workplaceCards.forEach(function(card, index) {
                    ...
                    if (name.includes(query) || address.includes(query)) {
                        ...
                        var nameEl = card.querySelector('.workplace-name');
