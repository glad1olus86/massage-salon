        ...
        <div class="col-lg-8 col-md-10 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h5>{{ __('Edit Shift Template') }}: {{ $shift->name }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('attendance.shifts.update', $shift) }}" method="POST">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="work_place_id" class="form-select @error('work_place_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($workPlaces as $wp)
                                        <option value="{{ $wp->id }}" {{ old('work_place_id', $shift->work_place_id) == $wp->id ? 'selected' : '' }}>
                                            {{ $wp->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" name="break_minutes" class="form-control @error('break_minutes') is-invalid @enderror" 
                                       value="{{ old('break_minutes', $shift->break_minutes) }}" min="0" max="480">
                                @error('break_minutes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="color" name="color" class="form-control form-control-color" 
                                       value="{{ old('color', $shift->color) }}" style="height: 38px; width: 100%;">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="pay_type" class="form-select @error('pay_type') is-invalid @enderror" required>
                                    <option value="per_shift" {{ old('pay_type', $shift->pay_type) == 'per_shift' ? 'selected' : '' }}>
                                        {{ __('Per Shift') }}
                                    </option>
                                    <option value="hourly" {{ old('pay_type', $shift->pay_type) == 'hourly' ? 'selected' : '' }}>
                                        {{ __('Hourly') }}
                                    </option>
                                </select>
                                @error('pay_type')
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" name="pay_rate" class="form-control @error('pay_rate') is-invalid @enderror" 
                                       value="{{ old('pay_rate', $shift->pay_rate) }}" min="0" step="0.01">
                                @error('pay_rate')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <div class="form-check">
                                    <input type="checkbox" name="night_bonus_enabled" class="form-check-input" 
                                           id="nightBonusEnabled" value="1" {{ old('night_bonus_enabled', $shift->night_bonus_enabled) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6 mb-3" id="nightBonusPercent" style="{{ old('night_bonus_enabled', $shift->night_bonus_enabled) ? '' : 'display: none;' }}">
                                <label class="form-label">{{ __('Night Bonus (%)') }}</label>
                                <input type="number" name="night_bonus_percent" class="form-control" 
                                       value="{{ old('night_bonus_percent', $shift->night_bonus_percent) }}" min="0" max="100" step="0.01">
                            </div>
                        </div>

                        <div class="d-flex justify-content-end gap-2 mt-4">
                            <a href="{{ route('attendance.shifts.index') }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
                            <button type="submit" class="btn btn-primary">{{ __('Update Shift Template') }}</button>
                        </div>
                    </form>
                </div>
            </div>
